...
def tri_box_intersection(bbmin, bbmax, tri_pre, debug=False):
    ...
    c = np.where(n > 0, dp, np.array([0., 0., 0.]))
    # vc = v[0,:]
    vc = tri_pre['cent']
    d1 = np.dot(n, c-vc)
    d2 = np.dot(n, (dp-c)-vc)
...
def main():
    ...
    for _ in range(args.trials):
        ...
        for ti in range(0, Ntris):
            hit[ti] = tri_box_intersection(
                bmin, bmax, tris_pre[ti], debug=True)
        ...
        if draw:
            ...
            for ti in range(0, Ntris):
                ...
                mlab.triangular_mesh(
                    *(pts.T), tris[ti][None, :], opacity=1, color=color)
