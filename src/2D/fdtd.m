...
draw = false; %to plot or not
add_dome = false; %add dome to scene
...
% in_mask(X(:)>=1.5 & Y(:)>=0.0 & X(:)<2 & Y(:)<3) = false;
clear X Y;
...
fprintf('size(u0) is %s\n', mat2str(size(u0)))
fprintf('size(in_mask) is %s\n', mat2str(size(in_mask)))
% fprintf('size(ib) is %s\n', mat2str(size(ib)))
% fprintf('size(Kib) is %s\n', mat2str(size(Kib)))
% fprintf('Kib(1,1) is %f\n', Kib(1,1))
% return
